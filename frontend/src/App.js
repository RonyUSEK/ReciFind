...
/**
 * Single ingredient pill that supports INCLUDE (+) and EXCLUDE (-) states.
 * Click on the button to cycle through: include -> exclude -> remove
 */
...
const App = () => {
  ...
  // State for Ingredient Selection: key=name, value='include' or 'exclude'
  const [activeIngredients, setActiveIngredients] = useState(new Map());
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedSuggestionIndex, setSelectedSuggestionIndex] = useState(-1);

  // Effect to apply 'dark' class to document element
  useEffect(() => {
  ...
  /**
   * Toggles the mode of an existing ingredient: include -> exclude -> remove.
   */
  ...
  /**
   * Removes an ingredient entirely.
   */
  ...
  // Dynamic classes for quick toggle buttons
        ...
        <section className="mb-8 sm:mb-12">
          ...
          <div className="relative max-w-3xl mx-auto shadow-xl rounded-2xl bg-white dark:bg-gray-800 mb-8 border border-gray-200 dark:border-gray-700">
            <div className="flex flex-col sm:flex-row sm:items-center p-2 gap-2">
              ...
              {/* Suggested Pill - Hidden on mobile when search term is active */}
